---
id: MV-S-001
titulo: "Os Primeiros Números (1 a 3)"
fase: "Sementes"
tipo: "Lição Sementes (Nível 0)"
versao: 3.6 (Inevitable Gold)
status: "Canônico"
guardia: "Celeste"
local: "A Trilha das Sementes"
tempo: "15 min"
meta: "💎 Conquistar o segredo do 1, do 2 e do 3 através do toque das mãos e do ritmo do corpo."
tgtb: "Math K, Lesson 1"
---

# 🌿 Os Primeiros Números (1 a 3)

> [!NOTE]
> O Gutenberg Engine extrairá o Guardião, Local, Tempo e Meta automaticamente para o cabeçalho.
> Mantenha o conteúdo abaixo limpo e focado na jornada.

---

> [!TIP]
> **Lembrete de Graça (Renew):**
> *Portador da Tocha: Este roteiro é um mapa, não uma lei. Se o coração do Viajante pedir outro caminho hoje, siga o amor, não o texto. O improviso amoroso vale mais que a leitura perfeita.*

## 📜 1. A Bancada (Mise-en-place)

> [!MESTRA]
> **A Liturgia da Ordem:**
> *   **O Conceito:** "Mise-en-place" significa "tudo no lugar". Se você levantar para buscar a vela, o **fio da atenção** se rompe.
>
> 🎯 **A Ideia Viva (O Segredo):**
> O Três não é apenas "mais que dois"; é a primeira família completa (Pai, Mãe, Filho). A multiplicidade começa com a unidade.
>
> 📦 **Vivência (Exploração Sensorial):**
> *   [ ] **1. O Concreto:** 3 sementes grandes (ou pedras, nozes, botões).
> *   [ ] **2. O Belo:** Uma bandeja limpa para dispor os elementos.
> *   [ ] **3. O Elo:** *Promessa da Aventura* (Convite verbal).
> *   [ ] **4. O Sagrado:** Uma vela (Símbolo da Presença) e fósforos.
>
> 🧵 **O Fio de Ouro:** Na última jornada, recebemos o Passaporte. Celeste quer vê-lo hoje. Ao final, guardaremos **uma** semente.

---

## 🎧 2. [AUDIO-SCRIPT] (Postura da Alma)

> [!TIP]
> **Portador da Tocha, ouça este breviário enquanto prepara o ambiente. Ele deve calibrar o seu coração para a jornada sacra.**
>
> "Sinta a vitalidade. Celeste é a curiosidade em movimento. Nesta lição, você é o guardião da descoberta vibrante. A Ideia Viva hoje é que o mundo pode ser contado um por um. Antes de chamar o Viajante, coloque as 3 sementes escondidas sob um pano ou em sua mão fechada. O segredo deste encontro reserva-se na surpresa da revelação... prepare-se para vibrar com cada novo número com ele."

---

## 🕯️ 3. O Ritual de Entrada (O Teatro da Mente)

> [!RITUAL]
> *[Ação: Acenda a vela, ligue a luz ou abra a cortina. A LUZ deve ser o foco.]*
>
> **Você diz:**
> `[tom de segredo]`
> "Eu sou o Portador da Tocha... Hoje, a luz nos guia para a trilha de Celeste. Você está pronto para os primeiros tesouros?"
>
> "Respire devagar... Deixe as pálpebras pesarem..."
>
> *[pausa de 3 segundos]*
>
> "Abra os olhos... Veja o rastro laranja de Celeste!"

---

## 🗺️ 4. A Jornada (Imersão Total)

> [!NARRATIVA]
> *[Cenário: Uma trilha de terra macia rodeada por carvalhos gigantes. Um vulto laranja passa correndo entre as árvores.]*
>
> **Narrador:**
> "Você abre os olhos. Celeste, a pequena raposa, está saltando sobre as raízes! Ela parou e está olhando para você com a pata cheia de algo precioso."
>
> **Celeste:**
> `[tom de voz vibrante e curioso]`
> *"Oi, Viajante! Veja o que o vento trouxe! O Rei deixou cair promessas de vida no chão... olhe para a minha pata... quantas sementes você vê?"*

---

## 💡 5. A Ideia Viva (O Segredo Revelado)

> [!CONCEITO]
> **Celeste:**
> *"Uma... duas... TRÊS! É como uma pequena família de sementes morando juntas. Cada uma é uma vida inteira esperando!"*
>
> `[pausa de maravilha]`
>
> **VOCÊ FAZ (O Toque da Matéria):**
> 1. Coloque 1 semente na palma da mão da criança. "Uma promessa viva."
> 2. Coloque a segunda. "Duas promessas de mãos dadas."
> 3. Coloque a terceira. "Três! Agora a mão está cheia de futuro."
> 4. `[tom de maravilhamento]` "Contemple como o Três é firme e completo!"

---

## 🧱 6. O Caminho Dourado (Mãos na Massa)

> [!ATIVIDADE]
> **A Missão Principal (Obrigatória):**
> `[tom de convite nobre]`
> *"Viajante, vamos construir o que Celeste nos mostrou?"*
>
> 1.  **Concreto:** Coloque as 3 sementes na mesa. Peça para a criança separá-las em grupos (1 e 2, ou 1, 1 e 1).
> 2.  **Pictórico:** Dê 3 folhas (reais ou desenhadas). Peça para desenhar 3 pontinhos dourados (sementes de luz) sobre elas.
> 3.  **Abstrato:** Peça para mostrar 3 dedos e escondê-los rápido. "Onde foi parar o três?".
>
> *[Nota de Virtude: Parabéns. Você ativou o toque, o olho e o símbolo.]*

---

## 🦋 7. Se Quiser Voar (Opcional)

> [!TIP]
> **Fez o Caminho Dourado? A missão está cumprida.**
> *Só siga adiante se os olhos do Viajante pedirem "Mais!".*
>
> **A Via do Corpo (Movimento Vivo):**
> *   **Ação:** Dê 3 pulos de sapo. A cada pulo, grite o número: "UM! DOIS! TRÊS!".
> *   **Desafio:** Correr até a parede e tocar nela 3 vezes rápidas.

---

## 🗣️ 8. Momento de Conversa (Narração da Alma)

> [!NARRAÇÃO]
> *[Guarde as sementes, mas deixe uma separada. Foco no olhar e na luz acesa.]*
>
> **Celeste pergunta:**
> *"Se você encontrasse 3 tesouros na floresta, para quem você daria cada um deles? O que você faria com tanto poder?"*
>
> `[pausa - respeite solenemente o tempo da criança]`
>
> **Sondas de Sabedoria (Use 1 ou 2):**
> *   "O que é mais pesado na sua mão: 1 semente ou 3?"
> *   "Como o 3 se parece quando está escondido nos seus dedos?"

---

## 🕊️ 9. Despedida & Bênção

> [!FECHAMENTO]
> **Celeste:**
> `[gesto de despedida nobre]`
> *"Você tem mãos de colhedor de luz, Viajante. Guarde o Três com carinho."*
>
> **(Entrega do Token):** *"Pegue esta semente. Guarde-a bem. Na próxima jornada, Bernardo precisará dela para abrir uma porta especial."*
>
> *"Na próxima jornada, Bernardo quer te mostrar a ordem nas janelas do Castelo."*

---

## 🌌 10. Ritual de Encerramento (Ordem & Bênção)

> [!RITUAL]
> **Você diz:**
> "O Reino adormece, mas a semente que guardamos continua viva em seu bolso e em seu coração."
> "Até a próxima jornada."
>
> *[Ação: Apaguem a vela, apague a luz ou fechem a cortina. A jornada termina no silêncio.]*

---

## 🏛️ 11. Por que isso importa? (A Cátedra dos Pais)

> [!NOTE]
> **A Autoridade para o Portador da Tocha:**
>
> 🧠 **O Cérebro da Criança (Neurociência & SGP):**
> O cérebro infantil não "lê" números, ele "sente" quantidades. Quando a criança segura 3 sementes, os receptores táteis enviam um sinal elétrico que solidifica o conceito de "quantidade discreta" (Subitização). Sem esse toque físico, o "3" seria apenas um rabisco sem alma. Você está construindo a estrada neural da matemática.
>
> 🌌 **A Ordem do Reino (Filosofia & Beleza):**
> *"A Trindade é a âncora de toda a realidade."* (Pérola Teológica). O Três é a primeira figura estável (triângulo). Ao apresentar o 3 com beleza, você mostra que o universo não é caos, mas uma estrutura confiável onde as coisas se agrupam em famílias.
>
> 🛡️ **Sua Confiança (Vacina contra a Culpa):**
> Portador, se hoje a criança derrubou as sementes ou não quis pular, lembre-se: a lição não falhou. Ela apenas tocou a realidade. O importante não foi a contagem perfeita, mas o tempo que vocês passaram juntos olhando para algo maior que vocês dois. A "Atenção Compartilhada" é o verdadeiro milagre.

---

## 🛡️ 12. Auditoria da Mestra (Veredito CM)

- [ ] **Atenção:** A criança ignorou o ambiente para olhar as sementes?
- [ ] **Hábito:** A troca de sementes foi feita com cuidado e silêncio?
- [ ] **Ideia Viva:** Ela percebeu que o '3' é mais que o '2'?
- [ ] **Narração:** Ela conseguiu nomear para quem daria os tesouros?
